...
set prev node to input node to be deleted 
set current node to prev.next 

while current has a next node 
  overwrite previous node's value with current node's value 
  advance previous to current 
  advance current to current.next 

(current node is now the last node)
overwrite prev's value with current's value 
set prev.next to null (to remove current/last node)
