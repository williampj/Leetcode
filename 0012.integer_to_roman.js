...
converter - nested array of strings 
position - int (reverse iterator)
digit - int 
...
converter[digit][position]:
  4 subarrays are positions
    10 elements with string of roman value at that position 
...
while number is not zero 
  ...
  prepend return of digit and position in converter to output
...
========== Solution ==========
Runtime 105 ms Beats 83.15% of users with JavaScript
Memory 53.90 MB Beats 90.23% of users with JavaScript

Time Complexity: O(N) - linear iteration through input digits 
Memory Complexity: O(N) - output string grows as input number gets larger
...
var intToRoman = function(num) {
  ...
  while (num > 0) {
      digit = num % 10;
      output = CONVERTER[position][digit] + output;
      num = (num - digit) / 10; 
      position += 1; 
  }
